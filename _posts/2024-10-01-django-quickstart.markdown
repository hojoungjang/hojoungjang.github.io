...
추가로 몇몇 기업에서 장고를 인터널 툴 개발할떄 사용하는 것 같은데 이 유즈케이스가 개인적으로 궁금해서 추후 더 알아보고 싶다.

# Django
Django (장고) 는 파이썬 웹 프레임워크이다. 장고를 사용하여 웹 애플리케이션의 프로트엔드 및 백엔드를 구성 할 수 있다. 구성요소가 꽤나 많고 그만큼 러닝커브는 가파르지만 프레임워크에서 기본적으로 제공하고 지원하는 부분이 많아서 규모가 있는 애플리케이션 개발시 이점이 될 것 같다.
...
* `urls.py` - 애플리케이션의 모든 라우트를 지정한다. 장고 앱 전체의 최상위 라우트 정의이고 각각의 장고 앱은 `urls.py` 따로 가지고 있고 독립적으로 URL 경로와 뷰를 관계를 정의해 준다.

ASGI 와 WSGI 는 파이썬 웹 프레임워크 앞단에 연결되는 웹서버의 종류인데 `asgi.py` 와 `wsgi.py` 는 웹 서버와 장고 코드를 연결해 서빙하는 역할을 한다.

### 장고 앱 만들기
장고 앱은 전체 웹 애플리케이션의 구성하는 한가지의 서비스/역할을 하는 요소 이다.
...
### Global URLconf 설정하기
프로젝트 root 앱에도 `urls.py` 가 존재합니다. 여기에 각 앱의 URLconf 를 포함시켜줍니다.
